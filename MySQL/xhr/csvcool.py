...
def read(stream, keys=None, encoding=None, sniffsize=100, **kwargs):
    ...
    if "dialect" not in kwargs:
        kwargs["dialect"] = csv.Sniffer().sniff(
            stream.read(sniffsize)
        )
        stream.seek(0)
    ...
    for idx, row in enumerate(csv.reader(stream, **kwargs)):
        if idx == 0 and keys == None:
            ...
            rows.append([format(c) for c in row])
...
def write(csvcool, stream, encoding=None, **kwargs):
    ...
    if encoding:
        format = lambda x: (
            ...
            else unicode(x, "utf-8").encode(encoding)
        )
